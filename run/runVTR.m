function [] = runVTR(cepOrder, fs_in, numFormants, trackBW, vtrDbNum)
...
% Interesting examples 200, 202, 204*, 210, 180 is really nice, 310 early
% on is nice
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
algFlag = [0 1 1 0]; % Select 1 to run, 0 not to
...
if(fs ~= fs_in)
    wav = resample(wav,fs,fs_in,1024);
...
wav = wav(1:wLength*wOverlap*length(trueStateWS));
...
if (trackBW)
    ...
    %Q = diag(var(trueStateWS(:,formantInds(:,1)==1),0,2));
    Q = diag(var(trueStateWS(:,2:end)-trueStateWS(:,1:end-1),0,2));
end
...
if algFlag(EKS_EM)
    ...
    maxNumIter = 15;  % Maximum number of EKS-EM iterations
...
plotSpecTracks2(wav, estTracks(:,:,2), aParams);
