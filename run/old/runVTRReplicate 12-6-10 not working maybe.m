...
cepOrder = 15; %20;
fs = 2*3500; %2*8000;
useCorr = 0; %0; % Flag for whether or not to use correlation
frmts = [1 2 3]; % if useCorr=1, specify formants to include in regression
useVAD = 1; % 0.3 s, all disp 0.6 s
numFormants = 3; %4; % to track
trackBW = 0;
vtrDbNum = 265;
zOrder = 0;
...
for ii = 1%:516 % praat bombs on 364
    ...
    %dataFileNameIn =
    %strcat('../data/VTRsynth/VTRsynth',num2str(vtrDbNum),'.wav');
