...
for jj = 1:numTrials
    ...
    [rmse{jj}, x_est{jj}, x_errVar{jj}, x{jj}, trueState{jj}] = runSynth_OLA(F, Fbw, Z, Zbw, N, snr_dB, ...
...
plotStateTracksFZ_EstVar_Truth(trueState{jj},x_est{trial},x_errVar{trial},titleCell,nP,trackBW)
...
plot(trueState{jj}', 'LineWidth', 3, 'Color', 'k')
xlabel('Frame')
ylabel('Frequency (Hz)')
...
plotStateTracksFZ_CI(trueState{jj},x_est,titleCell(:,[1 2]), nP, trackBW);
