...
Dengflag = 1; % 1 - Deng linearization, 0 - Jacobian of mapping
...
%%
% tic
for ii = vtrDbNum%1:516 % praat bombs on 364
    %%%%%%%%%%%%%%%%%%%%%%%%%% Load Data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    vtrDbNum = ii;
...
%     wav = wav(1:wLength*wOverlap*(length(trueStateVTR)+1));
    ...
    if (trackBW)
        ...
        %Q = diag(var(trueStateWS(:,formantInds(:,1)==1),0,2))/Qscale; %true variance PER TRACK
...
figure, plot(rmseAllS(1:end, 1, 3))
