...
rmseAllS = zeros(516, 3, 4); % speech only
...
for ii = 196 %1:516
    ...
    wavLen = round((numFrames+1)*wLengthMS/1000*fs/2);
    wav = wav(1:wavLen);
...
%     trueStateWS  = trueStateWS(1:numFormants,:);
%     bwDataWS     = bwDataWS(1:numFormants,:);
%     trueStateVTR = trueStateVTR(1:numFormants,:);
%     bwDataVTR    = bwDataVTR(1:numFormants,:);
...
firstline = get(get(gca,'Title'),'String');
title({['File: ', dataFileName]; firstline})
set(gca, 'PlotBoxAspectRatio', [12 1 1])
