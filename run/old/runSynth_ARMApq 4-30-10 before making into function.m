...
F = 500 + 1000*(0:(nP - 1)); % Initialize pole locations
Fbw      = 80 + 40*(0:(nP - 1));
Z    = 600 + 1000*(0:(nZ - 1));    % Initialize with offset from pole locations by 100Hz
Zbw      = 80 + 40*(0:(nZ - 1));
...
x0 = [F; Z]+400;
% x0 = 2000*ones(size(x0)); % same number on input
...
if algFlag(EKF)
    ...
    figure, plot(x_estEKF(1:length(F), :)', 'k')
    hold on, plot(x_estEKF(length(F)+1:length(F)+1+length(Z)-1, :)', 'b')
    xlabel('Frame number')
...
%% To do: figure out how close zeros and poles can be before tracker breaks
%% (depends on ARMAx.m actually)
%% modify number of cepstral coefficients to see what is necessary, given a
%% certain number of resonances or anti-resonances
%% modify tracker to track bandwidths
