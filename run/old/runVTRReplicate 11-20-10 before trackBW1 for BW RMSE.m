...
%     wav = wav(1:wLength*wOverlap*(length(trueStateVTR)+1));
    ...
    if(useVAD)
        ...
        load([dataFileName(1:end-4), '.mat'])
        frameInds = timitVAD(wav, data.phones, aParams, plotVad);
...
break
%% START HERE TO ADD TRACKING BW...SEE IF RMSE OF FORMANTS
%% DECREASES...REPORT BW RMSE BUT DON'T PUT TOO MUCH FAITH IN IT

% save('..\results\rmse_EKS_WS_Praat_trackBW0_cep12_ar12ma0_win20_pe07_fs7000_simpleVAD', 'rmseAll', 'rmseAllS')
% save('..\results\rmse_EKS_WS_Praat_trackBW0_cep12_ar12ma0_win20_pe07_fs7000_timitVAD', 'rmseAll', 'rmseAllS')
save('..\results\rmse_EKS_WS_Praat_trackBW1_cep12_ar12ma0_win20_pe07_fs7000_timitVAD', 'rmseAll', 'rmseAllS') % to do

%%
load('..\results\rmse_EKS_WS_Praat_trackBW0_cep12_ar12ma0_win20_pe07_fs7000_timitVAD', 'rmseAll', 'rmseAllS')
for ii = 1:3
    for jj = 1:4
        rmse_table(ii, jj) = mean(rmseAllS(:, ii, jj));
    end
end
...
plotSpecTracks2_estVar(wav, estTracks, x_errVarEKS, aParams, numAntiF);
firstline = get(get(gca,'Title'),'String');
title({['File: ', dataFileName]; firstline})
set(gca, 'PlotBoxAspectRatio', [10 1 1])
...
plotSpecTracks2_estVar_Truth(wav, estTracks, x_errVarEKS, trueStateVTR, aParams, numAntiF);
firstline = get(get(gca,'Title'),'String');
title({['File: ', dataFileName]; firstline})
set(gca, 'PlotBoxAspectRatio', [10 1 1])