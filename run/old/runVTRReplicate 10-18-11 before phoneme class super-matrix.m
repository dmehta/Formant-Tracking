...
%% Set parameters
% vtrDbNum = [1:77 79:463 465:516]; % for praat on VTRsynthf0
% vtrDbNum = [1:123 125:130 132:216 218:415 417:516]; % for praat on VTRsynthf03500
vtrDbNum = 1:516; % Choose VTR database number(s) to process from 1 to 516
...
cepOrder = 15;
fs = 2*3500; % if more than 7000, then get rid of code that reflects true trajectories below 3500 Hz
...
useVAD = 1; % 0.3 s, all disp 0.6 s
  VADflag = 3; % only if useVAD=1; 1 - percentile algorithm, 2 - TIMIT phones, 3 - phoneme classification
...
% output dir
savedir = '..\results\temp';
% savedir = '..\results\EKS_trackBW1_Realcep30_ar12ma0_win20_pe07_fs16000_timitVAD_useCorr1_Jacoblin_Q5e4bw1e4_BWws_ignore20';
...
rmseAll = zeros(length(vtrDbNum), length(numTrackers), numFormants+1); % database length X num algorithms (EKS, WS, Praat) X formant number (1, 2, 3, overall)
rmseAllS = zeros(length(vtrDbNum), length(numTrackers), numFormants+1); % same dims, but speech frames only
rmseAllC = zeros(length(vtrDbNum), length(numTrackers), numFormants+1, numClasses); % similar dims, but add number of classes

%%
% tic
for ii = vtrDbNum
% for ii = [1:152 154:192 194:516] % when doing praat
% for ii = [1:77 79:463 465:516]; % for praat on VTRsynthf0
% for ii = [153 193]
% for ii = 1
...
%         countTrack = countTrack + 1;
    ...
    for jj = numTrackers
        ...
        if VADflag == 3
            rmseC    = zeros(numFormants, sum(algFlag), numClasses);
            rmseAllC = zeros(numFormants, sum(algFlag), numFormants+1, numClasses);
            c = 0;
            for j = 1:numFormants
                % Compute errors for particular phoneme classes
                % class = vowel (1), semivowel (2), nasal (3), fricative (4),
                % affricate (5), stop (6)
                for class = 1:numClasses
                    Cinds = find(frameInds == class);
                    rmseC(j,countTrack,class) = sqrt(mean((estTracks2(j,Cinds(startupframes+1:end),countTrack)-trueStateVTR2(j,Cinds(startupframes+1:end))).^2)); % ignore startup
                    rmseAllC(vtrDbNum,trackerType,j,class) = rmseC(j,countTrack,class);

                    cDisp  = sprintf('Formant %d, Class %d, RMSE: %2.2f\n', j, class, rmseC(j,countTrack,class));
                    c = [c cDisp];
                end
            end
...
if ~strcmpi(savedir,''), save(savedir, 'rmseAll', 'rmseAllS', 'rmseAllC'), end
