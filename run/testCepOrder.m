...
F = [500 1500 2500 3000]'; % in Hz, time-invariant formant center frequency (F)
Fbw = [50 80 120 50]'; % in Hz, time-invariant formant bandwidth (Fbw)
...
for ii = 1:length(variableParam)
    %disp(['Parameter value: ', num2str(variableParam(ii)), '...'])
    for jj = 1:numTrials
    %disp(['Monte Carlo Trial #', num2str(jj), '...'])
    %fprintf('.')

    % comment if no variable parameter
    %cepOrder = [variableParam(ii), variableParam(ii)];
