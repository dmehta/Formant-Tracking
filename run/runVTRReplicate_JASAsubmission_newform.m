...
% final runs
% VTR, source 1:
% savedir = '..\results\EKS_WS_Praat_trackBW0_ARMAcep15_ar12ma0_win20_pe07_fs7000_timitVAD_useCorr0_Jacoblin_Q5e4bw1e4_BWfix';
...
% savedir = '..\results\EKF_WS_Praat_trackBW1_ARMAcep15_ar12ma0_win20_pe07_fs7000_timitVAD_useCorr1_Jacoblin_Q5e4bw1e4_BWfix';
% savedir = '..\results\EKF_WS_Praat_trackBW0_ARMAcep15_ar12ma0_win20_pe07_fs7000_timitVAD_useCorr1_Jacoblin_Q5e4bw1e4_BWfix';
% savedir = '..\results\EKS_WS_Praat_trackBW1_ARMAcep15_ar12ma0_win20_pe07_fs7000_timitVAD_useCorr1_Jacoblin_Q10e4bw1e4_BWfix_ignore20';
...
% vtrDbNums = [1:152 154:192 194:516]; % when doing praat on VTR, Praat bombs on 153 and 193, run with sl algorithm
...
startupframes = 20; % 20 chosen by looking at VTRsynth1 output; put back to 0 for JASA resubmission
...
rmseAllSwrong = zeros(length(vtrDbNums), numTrackers, numFormants_error+1); % same dims, but speech frames only
...
%         params.ds_freq = fs;
%     [trueStateWS, bwDataWS] = getFormantTrackWS(dataFileNameIn, fs_in, WS_dir, params);
...
%         countTrack = countTrack + 1;
    ...
    rmseSwrong    = zeros(numFormants_error, sum(algFlag));
    ...
    for jj = trackersToRun
            ...
            case 3
                ...
                params = getFormantParamsDefault;
                 ...
                 if (source == 1 && (vtrDbNum == 153 || vtrDbNum == 193)) || ...
                     ...
                     (source == 5 && (vtrDbNum == 124 || vtrDbNum == 131 || vtrDbNum == 217 || vtrDbNum == 416))
                     params.method = 'sl';
                 end
        ...
        %Compute and Display MSE and RMSE
            ...
            rmseAllSwrong(vtrDbNum, trackerType, numFormants_error+1) = mean(rmseSwrong(:,countTrack));

            sDisp  = sprintf('Average   RMSE: %2.2f; NS RMSE: %2.2f\n', mean(rmse(:,countTrack)),mean(rmseSwrong(:,countTrack)));
            s = [s sDisp];
        ...
        if VADflag == 3
            ...
            for j = 1:numFormants_error
                ...
                for class = 1:numClasses
                    Cinds = find(frameInds == class); % ignore startup here, if desired
...
for ii = trackersToRun % # trackers
    for jj = 1:4 % # rmse values
        tmp = rmseAllSwrong(indices, ii, jj);
        rmse_each_table(ii, jj) = mean(tmp(~isnan(tmp))); % take into account any NaNs
    end
end

figure, plot(indices, rmseAllSwrong(indices, 1, 4), 'o')
...
% Number of frames for each tracker
temp = trueStateVTR2_All_Db_f;disp(length(temp))
...
%% cycle through VTR database and save gender (female flag) to TIMITgender.mat
% vtrDbNums = 1:516;
