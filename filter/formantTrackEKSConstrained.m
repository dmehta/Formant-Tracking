function [m_upS, m_upSC, P_upS, PP_upS,lgLkl] = formantTrackEKSConstrained(y, F, Q, R, x0, formantInds, fs, bwStates, smooth)
...
for k = 1:N
    ...
    % Now recompute m_upC(:,k) by constraining this thing
    if(k > 1)
...
BigA = zeros(numF*N,numF*N);
for i = 1:N
   BigA(numF*(i-1)+1:numF*i,numF*(i-1)+1:numF*i) = A; 
end
...
cvx_quiet(true);
variable x(numF*N);
minimize( (x-mVec)'*(x-mVec));
subject to
BigA*x < Biga;
cvx_end

m_upSC = reshape(x,numF,N);
