% specifically called by runWaveZ_nan_synth9v.m
...
for k = 1:N
    ...
    if any(~formantInds(:,k)) % then the state vector length has to be modified
        % here I specifically write code for /piano/: 787 frames, 452:581
        % is /n/; when 0 detected in formantInds, kill antiformant parameters
        
    % Linearize about state (uses Taylor expansion)
    if trackBW
        curFVals = m_pred([1:numF],k);
        curBVals = m_pred([numF+1:2*numF],k);
    ...
    % Compute gain and knock out unobservables (assumes BW tracking!)
    S = H*P_pred(1:2*numF,1:2*numF,k)*H' + R;
    %K = (mask*P_pred(1:2*numF,1:2*numF,k)*H')/S; % K is the gain
    K = (P_pred(1:2*numF,1:2*numF,k)*H')/S; % K is the gain
    ...
    % Update steps
    m_up(:,k)   = [abs(m_pred([1:2*numF],k) + K*e); NaN; NaN; NaN; NaN];
    
    Ptemp = P_pred(1:2*numF,1:2*numF,k) - K*H*P_pred(1:2*numF,1:2*numF,k);
    Ptemp(2*numF+1:2*numF+2*numAntiF, :) = NaN;
    Ptemp(:, 2*numF+1:2*numF+2*numAntiF) = NaN;
    P_up(:,:,k) = Ptemp;

   % Compute prediction steps for all but the last step
    if k < N
        m_pred(:,k+1)   = [F(1:2*numF,1:2*numF)*m_up([1:2*numF],k); NaN; NaN; NaN; NaN];
       
        Ptemp = F(1:2*numF,1:2*numF)*P_up(1:2*numF,1:2*numF,k)*F(1:2*numF,1:2*numF)' + Q(1:2*numF,1:2*numF);
        Ptemp(2*numF+1:2*numF+2*numAntiF, :) = NaN;
        Ptemp(:, 2*numF+1:2*numF+2*numAntiF) = NaN;
        P_pred(:,:,k+1) = Ptemp;
    end
    ...
    if k == 452 % here's where state has to be reaugmented
        m_pred(2*numF+1:2*numF+2*numAntiF,k) = x0(2*numF+1:2*numF+2*numAntiF);
        %         m_pred(2*numF+1:2*numF+2*numAntiF,k) = [1600 50]';
        P_pred(2*numF+1:2*numF+2*numAntiF,:,k) = 0;
        P_pred(:,2*numF+1:2*numF+2*numAntiF,k) = 0;
        P_pred(2*numF+1:2*numF+2*numAntiF,2*numF+1:2*numF+2*numAntiF,k) = Pinit(2*numF+1:2*numF+2*numAntiF,2*numF+1:2*numF+2*numAntiF);
    end
        
    % START HERE FOR REGULAR CASE Linearize about state (uses Taylor expansion)
    if trackBW
...
% UNCOMMENT IF SHOWING TEST CASE OF /nan/        
    end
