...
for ii = 1:cepOrder
    ...
    for jj = 2*numFormants + 1:2*numFormants + numAntiformants
        bw = exp(-pi * ii * bwVals(jj-2*numFormants+numFormants)/fs);
        H(ii,jj) =  4*pi/fs*bw*sin(2*pi*ii*freqVals(jj-2*numFormants+numFormants)/fs);
    end
    ...
    for jj = 2*numFormants + numAntiformants + 1:length(freqVals)+length(bwVals)
        bw = exp(-pi * ii * bwVals(jj-2*numFormants-numAntiformants+numFormants)/fs);
        H(ii,jj) =  2*pi/fs*bw*cos(2*pi*ii*freqVals(jj-2*numFormants-numAntiformants+numFormants)/fs);
    end    
end